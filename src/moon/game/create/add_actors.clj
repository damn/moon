(ns moon.game.create.add-actors
  (:require [moon.entity.state.player-item-on-cursor :as player-item-on-cursor]
            [moon.game.create.entity-info-window-config :as entity-info-window-config]
            [moon.game.create.hp-mana-bar-config :as hp-mana-bar-config]
...
(defn step [{:keys [ctx/stage] :as ctx}]
  (let [config (.config stage)]
    (doseq [actor [((:dev-menu config)
                    ctx
                    rebuild-actors!
                    (requiring-resolve 'moon.game.create.world/step)
                    (requiring-resolve 'moon.game.open-editor/do!))
